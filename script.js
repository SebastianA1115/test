window.addEventListener('DOMContentLoaded', () => {
    ...
    const savedServersBox = document.getElementById('saved-servers-box');
    ...
    const btnSwitchServer = document.getElementById('btn-switch-server');
    ...
    if (roomIdParam) {
        ...
        // If server credentials exist, auto-join
        const savedHistory = JSON.parse(localStorage.getItem('fs_server_history') || '{}');
        if (savedHistory[roomIdParam]) {
            enterRoom(roomIdParam, savedHistory[roomIdParam].name);
        }
    }

    // Server memory manager
    function registerServerInHistory(id, name, passcode) {
        let history = JSON.parse(localStorage.getItem('fs_server_history') || '{}');
        history[id] = { name: name, passcode: passcode };
        localStorage.setItem('fs_server_history', JSON.stringify(history));
        renderSavedServersList();
    }

    function renderSavedServersList() {
        const history = JSON.parse(localStorage.getItem('fs_server_history') || '{}');
        const keys = Object.keys(history);
        
        if (keys.length === 0) {
            savedServersBox.classList.add('hidden');
            return;
        }

        savedServersBox.classList.remove('hidden');
        savedServersList.innerHTML = '';

        keys.forEach(id => {
            const server = history[id];
            ...
            div.innerHTML = `
                ...
                <button class="btn-launch-server" data-id="${id}">Quick Launch</button>
            `;
        ...
        document.querySelectorAll('.btn-launch-server').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                enterRoom(targetId, history[targetId].name);
            });
        });
    }

    // Sync global availability changes to the active room
    function syncGlobalAvailabilityToRoom(roomId) {
        if (!localUserIdentity) return;
        
        const globalSchedules = JSON.parse(localStorage.getItem(`fs_global_sched_${localUserIdentity.toLowerCase()}`) || '{}');
        const roomData = JSON.parse(localStorage.getItem(roomId) || '{"schedules":{}}');
        
        // Remove prior profile mappings inside the target room to sync cleanly
        if (roomData.schedules) {
            Object.keys(roomData.schedules).forEach(key => {
                if (roomData.schedules[key].name.toLowerCase() === localUserIdentity.toLowerCase()) {
                    delete roomData.schedules[key];
                }
            });
        } else {
            roomData.schedules = {};
        ...
        // Write the local user profile entries directly to the server
        Object.keys(globalSchedules).forEach(key => {
            roomData.schedules[key] = globalSchedules[key];
        });
    ...
    document.getElementById('btn-create-room').addEventListener('click', function() {
        ...
        localStorage.setItem(roomId, JSON.stringify(roomData));
        registerServerInHistory(roomId, name, passcode);
    ...
    document.getElementById('btn-join-room').addEventListener('click', function() {
        ...
        if (savedRoomRaw) {
            ...
            if (roomData.metadata.passcode === inputPasscode) {
                registerServerInHistory(roomId, roomData.metadata.name, inputPasscode);
    ...
    // Launch Test Server Event Handler
    document.getElementById('btn-test-room').addEventListener('click', function() {
        ...
        registerServerInHistory(testRoomId, testRoomName, testPasscode);
    ...
    function enterRoom(roomId, roomName) {
        ...
        // Auto-inject your global updates into this workspace upon entering
        syncGlobalAvailabilityToRoom(roomId);
    ...
    form.addEventListener('submit', function(e) {
        ...
        // 1. Write update to profile's global schedule memory space
        const globalSchedKey = `fs_global_sched_${localUserIdentity.toLowerCase()}`;
        const globalData = JSON.parse(localStorage.getItem(globalSchedKey) || '{}');
        globalData[id] = entryData;
        localStorage.setItem(globalSchedKey, JSON.stringify(globalData));

        // 2. Write to active room dataset
        const roomData = JSON.parse(localStorage.getItem(currentRoomId));
    ...
    function deleteEntry(id) {
        ...
        // Remove from current active server
        const roomData = JSON.parse(localStorage.getItem(currentRoomId));
        if (roomData && roomData.schedules) {
        ...
        // Remove from your local user profile memory
        const globalSchedKey = `fs_global_sched_${localUserIdentity.toLowerCase()}`;
        const globalData = JSON.parse(localStorage.getItem(globalSchedKey) || '{}');
        if (globalData[id]) {
